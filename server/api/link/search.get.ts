...
export default eventHandler(async (event) => {
  ...
  try {
    while (true) {
      const { keys, list_complete, cursor } = await URL.list({
        prefix: `link:`,
        limit: 1000,
        cursor: finalCursor,
      })
        ...
        for (const key of keys) {
          try {
            if (key.metadata?.url) {
              ...
              const { metadata, value: link } = await URL.getWithMetadata(key.name, { type: 'json' })
              if (link) {
                list.push({
                  slug: key.name.replace('link:', ''),
                  url: link.url,
                ...
                await URL.put(key.name, JSON.stringify(link), {
